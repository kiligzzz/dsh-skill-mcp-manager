  ...
  factory: (require) => {
    ...
    function McpPage() {
      ...
      const rows = servers.map((s) => {
        ...
        const scopeLabel = st.scope === "session" && s.enabled ? "按需加载" : String(toolList.length || st.tools || 0) + " tools";
        return React.createElement("div", { key: s.name, className: "cm-item" },
          React.createElement("div", { className: "cm-item-row" },
            ...
            React.createElement("button", { className: "cm-ico", title: "重连活跃会话中已加载的服务", onClick: () => refreshOne(s.name) }, "⟳"),
          ...
          expanded === s.name ?
            React.createElement("div", { className: "cm-toolbox" },
              toolList.length
                ...
                : React.createElement("div", { className: "cm-src" }, "工具按会话加载，不在设置页建立全局连接。"),
            ) : null,
        );
      });
